...
这表示本课把 `lesson-led` 解析到 board DTS 中已有的 `red_led`，
...
其中 `red_led:` 是节点 label。overlay 可以用 `&red_led` 引用这个节点。
...
```text
led.port     = GPIOA device
led.pin      = 1
led.dt_flags = GPIO_ACTIVE_LOW
```

所以应用不需要写 `GPIOA`、`1` 或有效电平。硬件变更优先放在 DTS/overlay。
...
1. 把 `lesson-led` 从 `&red_led` 改成 `&blue_led`，重新构建并观察串口输出。
